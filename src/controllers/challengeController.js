...
// Helper to check if request is in Arabic
const getIsAr = (req) => {
  ...
  try {
    ...
    const categoriesWithLock = (categories || []).map(cat => ({
      ...
      ar: {
        ...
        unlockRequirement: cat.level > (child?.level || 1)
          ? `أكمل جميع تحديات المستوى ${cat.level - 1} لفتح القفل.`
          : 'أكمل التحدي السابق لفتح القفل.'
      }
    }));
...
const startSession = async (req, res) => {
  try {
    ...
    if (!challengeCategoryId) return res.status(400).json({ success: false, message: 'challengeCategoryId is required' });
    ...
    if (error) return res.status(500).json({ success: false, message: 'Failed to start session', error: error.message });
...
const getNextQuestion = async (req, res) => {
  try {
    ...
    // Calculate answer length dynamically
    const calcLen = (ans) => {
      if (!ans) return 0;
      if (Array.isArray(ans)) return ans.length;
      if (typeof ans === 'string') return ans.length;
      return 0;
    };

    const ansEn = question.answer_en || question.answer;
    const ansAr = question.answer_ar || question.answer;
    ...
    res.status(200).json({
      ...
      en: {
        ...
        answerLength: calcLen(ansEn)
      },
      ar: {
        question: question.question_ar || question.question,
        options: question.options_ar || question.options,
        answerLength: calcLen(ansAr)
      }
    });
  } catch (error) {
    console.error("Get Next Question Error", error);
...
const submitAnswer = async (req, res) => {
  try {
    const { sessionId } = req.params;
    let { questionId, answer } = req.body;
    const isAr = getIsAr(req); // Check which language the app is submitting in
    ...
    const { data: question } = await supabase
      .from('challenge_questions')
      .select('answer, answer_en, answer_ar, time_limit_sec')
      .eq('id', questionId)
      .maybeSingle();

    if (!question) return res.status(404).json({ success: false, message: 'Question not found' });

    // Determine the expected correct answer based on the language
    const expectedAnswer = isAr ? (question.answer_ar || question.answer) : (question.answer_en || question.answer);

    // FLUTTERFLOW COMPATIBILITY LAYER
    // We try to parse the answer regardless of what the expected answer is.
    // This turns "\"عيسى\"" into "عيسى", and "[[0,0]]" into [[0,0]].
    if (typeof answer === 'string') {
      try {
        answer = JSON.parse(answer);
      } catch (e) {
        // If JSON.parse fails, it means it was just a raw normal string without quotes. 
        // We catch the error silently and leave 'answer' as it is.
      }
    }
    ...
    const attemptNumber = latestAttempt ? latestAttempt.attempt_number + 1 : 1;
    const timeStarted = latestAttempt && latestAttempt.time_started ? new Date(latestAttempt.time_started) : new Date();
    const now = new Date();
    const timeTakenSec = Math.round((now - timeStarted) / 1000);
    ...
    // If they are both simple text strings, compare them directly and trim spaces just to be safe!
    if (typeof answer === 'string' && typeof expectedAnswer === 'string') {
        isCorrect = answer.trim() === expectedAnswer.trim();
    } else {
        // If they are arrays or objects (like the matching games), stringify them for deep comparison.
        isCorrect = JSON.stringify(expectedAnswer) === JSON.stringify(answer);
    }
    ...
    if (error) return res.status(500).json({ success: false, message: 'Failed to submit answer' });
    ...
    res.status(200).json({
      ...
      timeTakenSec: timeTakenSec,
      attemptNumber,
      penalties: { timePenalty, repeatPenalty },
      canRetry: !isCorrect
    });
  } catch (error) {
    console.error('Exception in submitAnswer:', error);
...
const finishSessionInternal = async (req, res, sessionId, session) => {
    ...
    const { data: category } = await supabase
      .from('challenge_categories')
      .select('pass_percentage, name, name_en, name_ar')
      .eq('id', session.category_id)
      .single();
    ...
    if (passed) {
      ...
      else if (diffDays !== 0) newStreak = 1;
      ...
      // Bilingual Challenge Notification
      const catNameEn = category.name_en || category.name;
      const catNameAr = category.name_ar || category.name;

      await supabase.from('child_notifications').insert({
          child_id: session.child_id,
          type: 'challenge_completed',
          title: 'Challenge Mastered!',
          message: `You completed the ${catNameEn} challenge and earned ${starsEarned} stars!`,
          title_en: 'Challenge Mastered!',
          message_en: `You completed the ${catNameEn} challenge and earned ${starsEarned} stars!`,
          title_ar: 'تم إنجاز التحدي!',
          message_ar: `لقد أكملت تحدي ${catNameAr} وحصلت على ${starsEarned} نجمة!`,
          read: false
      });
      ...
      if (ach) {
         ...
         if (!hasAch) {
             await supabase.from('child_achievements').insert({ child_id: session.child_id, achievement_id: ach.id });
             
             // Bilingual Achievement Notification
             await supabase.from('child_notifications').insert({
                child_id: session.child_id,
                type: 'achievement_unlocked',
                title: 'Achievement Unlocked!',
                message: 'You earned the Challenge Seeker badge!',
                title_en: 'Achievement Unlocked!',
                message_en: 'You earned the Challenge Seeker badge!',
                title_ar: 'إنجاز جديد!',
                message_ar: 'لقد حصلت على شارة باحث التحديات!',
                read: false
             });
         }
      }
      ...
      if (promotionResult.promoted) {
          ...
          // Bilingual Level Up Notification
          await supabase.from('child_notifications').insert({
             child_id: session.child_id,
             type: 'level_up',
             title: 'Level Up!',
             message: `Congratulations! You have promoted to Level ${finalLevel}!`,
             title_en: 'Level Up!',
             message_en: `Congratulations! You have promoted to Level ${finalLevel}!`,
             title_ar: 'مستوى جديد!',
             message_ar: `تهانينا! لقد تم ترقيتك إلى المستوى ${finalLevel}!`,
             read: false
          });
      }
    ...
    return res.status(200).json({
      sessionId,
      finalScore,
      scoring: { totalScore: finalScore, timePenalty: timePenaltyTotal, repeatAttemptPenalty: repeatAttemptPenaltyTotal },
      starsEarned,
      passed,
      celebration: { sfxId: sfx?.id || null, animationId: animation?.id || null },
      newProgress
    });
};
...
const finishSession = async (req, res) => {
  try {
    ...
    if (session.status !== 'active') return res.status(409).json({ success: false, message: 'Session already completed.' });
